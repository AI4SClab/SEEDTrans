...
class wavelet_decomp(nn.Module):
    ...
    def create_W(self, P, is_l, is_comp=False):
        ...
        for i in range(0, P):
            filter_index = 0
            for j in range(i-5, P): #len(filter_list) = 8，要使分解对齐，权重矩阵需要有4个偏差
                if j<0:
                    filter_index += 1
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        return res_s, attn#, x_h, res_h# x_h, res_h
...
class Encoder(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        return x, attns#, h_freqs#h_freqs
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None): 
        ...
        #h_freqs = []
        #for layer in self.layers:
            #x, h3 = layer(x, cross, x_mask, cross_mask)
            #h_freqs.append(h3)
